...
extern CRITICAL_FLAGS critic_flags;
// extern MISSION_STATUS mission_status;

int pet_counter = 0;

// Helper function to reset COM MCU and send handshake
void reset_com_and_send_handshake(void) {
  uint8_t data[7] = {0x53, 0x01, 0x02, 0x03, 0x04, 0x7e, '\0'};

  // Reset COM system
  syslog(LOG_DEBUG, "WDOG timeout.. Resetting COM MCU");
  ...
  close(fd_com);
  pet_counter = 0; // Reset pet counter
}
...
static int watchdog_task(int argc, char *argv[]) {
  ...
  while (1) {
    if (pet_counter <= 630) {
      ...
      // If any mission is active and counter exceeds limit, reset the system
