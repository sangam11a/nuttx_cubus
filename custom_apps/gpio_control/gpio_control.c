...
{
  printf("Starting GPIO Control application...\n");
  printf("Pin Name: %s Pin number %d\n", argv[1], atoi(argv[2]));
  if(argc < 3){
  ...
  }else if(!strcmp(argv[1], "MSN1")){ //ADCS
    gpio_write(GPIO_MSN_3V3_EN, pin_mode);
    gpio_write(GPIO_MSN1_EN, pin_mode);
  }else if(!strcmp(argv[1], "MSN2")){ //CAM
    gpio_write(GPIO_MSN_3V3_EN, pin_mode);
    gpio_write(GPIO_MSN2_EN, pin_mode);
  }else if(!strcmp(argv[1], "MSN3")){ //EPDM
    gpio_write(GPIO_MSN_3V3_EN, pin_mode);
    gpio_write(GPIO_MSN3_EN, pin_mode);
  }else{  //keep on adding other gpio pins as you go
