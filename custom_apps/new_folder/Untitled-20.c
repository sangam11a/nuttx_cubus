        ...
        case OBC_MCU: /*
          ...
          { // __file_operations.cmd = COM_RX_DATA[HEADER + 1];
            ...
            if (cmds[0] == 0xCA || cmds[0] == 0x1D)
            {
              ...
              char filename[9][30] = {"/flags.txt", "/satHealth.txt", "/satHealth.txt", "/reservation_command.txt", "/cam_rgb.txt", "/epdmNew.txt", "/adcs_new.txt", "/cam_nir.txt", "/digipeater.txt"};
              ...
              else if ((cmds[2] == 0xF7))
              {
                ...
                strcat(__file_operations.filepath, "/adcs_new.txt");
              }
              else if ((cmds[2] == 0xF8))
              {
...
void truncate_text_file(struct FILE_OPERATIONS *file_operations)
{
  ...
  else if (strcmp(file_path, "/mnt/fs/mfm/mtd_mission/adcs_new.txt") == 0)
  {
...
void track_read_seek_pointer(struct FILE_OPERATIONS *file_pointer, int8_t seek_pointer[16])
{
  ...
  // if (readBytes > 0)
  {
    if (file_pointer->filepath == "/mnt/fs/mfm/mtd_mainstorage/satHealth.txt")
      ...
      index = 4 * 2;
    else if (file_pointer->filepath == "/mnt/fs/mfm/mtd_mission/adcs_new.txt")
      index = 3 * 4;
    else
...
//     int fd = file_open(&file_pointer, "/mnt/fs/sfm/mtd_mainstorage/adcs_new.txt", O_RDONLY);
...
//     cam[counter1++] = '\0';
//     sleep(1);
//     mission_data("/mtd_mainstorage/adcs_new.txt", &cam, counter1);
//     mission_data("/mtd_mission/adcs_new.txt", &cam, counter1);
//     // fd = file_open(&file_pointer, "/mnt/fs/mfm/mtd_mainstorage/adcs_new.txt", O_CREAT | O_RDWR | O_APPEND);
//     // file_write(&file_pointer, cam, strlen(cam));
//     // file_close(&file_pointer);
//     // close(fd);
//     // fd = file_open(&file_pointer, "/mnt/fs/mfm/mtd_mission/adcs_new.txt", O_CREAT | O_RDWR | O_APPEND);
//     // file_write(&file_pointer, cam, strlen(cam));
//     // file_close(&file_pointer);
//     // close(fd);
...
//       MISSION_STATUS.ADCS_MISSION = true;
//       strcpy(file_name, "/adcs_new.txt\0");
...
//                       {
//                         struct file fp;
//                         // int fd = file_open(&fp, "/mnt/fs/sfm/mtd_mission/adcs_new.txt", O_CREAT | O_RDWR);
//                         // if (fd < 0)
//                         // {
